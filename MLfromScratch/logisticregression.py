...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec  6 11:16:03 2017
...
from ml_sample import train_test_split
...
def logistic_log_likelihood_i(x_i,y_i,beta):
    if y==1:
        ...
        return 1-math.log(logistic(la.dot(x_i,beta)))

def logistic_log_likelihood(x,y,beta):
    return sum(logistic_log_likelihood_i(x_i,y_i,beta)
                    for x_i, y_i in zip(x,y))
    
def logistic_partial_ij(x_i,y_i,beta,j):
    return (y_i - logistic(la.dot(x_i, beta)))*x_i[j]

def logistic_log_grad_i(x_i,y_i,beta):
    return [logistic_partial_ij(x_i,y_i,beta,j)
            for j, _ in enumerate(beta)]

def logistic_gradient(x,y,beta):
    return reduce(la.vec_add,
                  [logistic_log_grad_i(x_i,y_i,beta)
                  for x_i,y_i in zip(x,y)])
